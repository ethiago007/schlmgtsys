...
import { getGradesByStudent, getFeesByStudent, getCollection } from '../../firebase/firestore'
import { gradeColor, formatCurrency } from '../../utils/constants'
import {
  MdGrade,
  MdAttachMoney,
  MdEventNote,
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [myGrades, myFees, allAttendance] = await Promise.all([
        ...
        console.error('Failed to load portal home', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const quickCards = [
    ...
    },
    {
      ...
      value: pendingFees.length > 0 ? `${pendingFees.length} unpaid` : 'All clear',
      path:  '/portal/fees',
    },
  ]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          {recentGrades.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">No grades yet</p>
          ) : (
              ...
              {recentGrades.map(g => (
                <div key={g.id} className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{g.term} · {g.session}</p>
                  </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm text-gray-700">
                ...
                Present: <span className="font-bold">{attendanceSummary.present}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-700">
                <MdCancel className="text-red-500" size={16} />
                Absent: <span className="font-bold">{attendanceSummary.absent}</span>
              </div>
              {attendanceRate < 75 && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-yellow-800">
                You have {pendingFees.length} unpaid fee{pendingFees.length > 1 ? 's' : ''}
              </p>
              <p className="text-xs text-yellow-600 mt-0.5">
                Total outstanding: {formatCurrency(pendingFees.reduce((s, f) => s + f.amount, 0))}
              </p>
            </div>
          </div>
        </motion.div>
