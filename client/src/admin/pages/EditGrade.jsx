...
const EditGrade = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [grade, studentList] = await Promise.all([
          getDocument('grades', id),
        ...
        reset({
          ...grade,
          classTest: String(grade.classTest),
          midTerm:   String(grade.midTerm),
          exam:      String(grade.exam),
        })
      } catch (error) {
  ...
  const onSubmit = async (data) => {
    if (parseFloat(data.classTest) > 20) return toast.error('Class test max is 20')
    ...
    try {
      ...
      const total = parseFloat(data.classTest) + parseFloat(data.midTerm) + parseFloat(data.exam)
      const { grade, remark } = getGrade(total)

      await updateDocument('grades', id, {
        ...data,
  ...
  if (fetching) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-gray-400 text-sm">Loading grade data...</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/admin/grades')} className="text-gray-500 hover:text-gray-800 transition">
          <MdArrowBack size={22} />
        </button>
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Edit Grade</h2>
          <p className="text-sm text-gray-500">Update grade record</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm text-gray-600 mb-1">Class Test (max 20)</label>
                <input
                  {...register('classTest')}
                  type="number" min="0" max="20"
                  className={inputClass(errors.classTest)}
                />
                {errors.classTest && <p className="text-red-500 text-xs mt-1">{errors.classTest.message}</p>}
              </div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">Mid Term (max 20)</label>
                <input
                  {...register('midTerm')}
                  type="number" min="0" max="20"
                  className={inputClass(errors.midTerm)}
                />
                {errors.midTerm && <p className="text-red-500 text-xs mt-1">{errors.midTerm.message}</p>}
              </div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">Exam (max 60)</label>
                <input
                  {...register('exam')}
            ...
            >
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-gray-800">
                  {preview.total}<span className="text-sm text-gray-400">/100</span>
                </p>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-semibold text-gray-700">{preview.remark}</p>
              </div>
            </motion.div>
          )}
