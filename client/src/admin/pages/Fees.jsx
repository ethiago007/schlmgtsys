    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{fees.length} fee records</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          ...
          <p className="text-3xl font-bold text-green-600 mt-1">{formatCurrency(totalCollected)}</p>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <p className="text-3xl font-bold text-yellow-600 mt-1">{formatCurrency(totalPending)}</p>
        </motion.div>
      </div>
      ...
      >
        {loading ? (
          <div className="p-10 text-center text-gray-400 text-sm">Loading fees...</div>
        ) : filtered.length === 0 ? (
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((fee, index) => (
                  <tr key={fee.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 text-gray-600">{fee.studentClass}</td>
